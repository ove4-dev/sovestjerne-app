...
export async function POST(request: Request) {
  ...
  if (story.status !== 'approved') {
    return NextResponse.json({ error: 'Historien må godkjennes før sending.' }, { status: 400 });
  }

  const childData = Array.isArray(story.children) ? story.children[0] : story.children;
  const parentEmail = childData?.parent_email;
  const childName = childData?.child_name || 'barnet ditt';

  if (!parentEmail) {
    return NextResponse.json({ error: 'Mangler e-post til forelder.' }, { status: 400 });
  }
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;background:#fff8ea;color:#0b1220;">
      ...
      <div style="background:white;margin-top:20px;padding:26px;border-radius:20px;border:1px solid #eee;">
        <h2 style="margin-top:0;color:#0b1d4f;">${story.title}</h2>
        <p>Hei!</p>
